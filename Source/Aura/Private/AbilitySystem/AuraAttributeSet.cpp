...
{
	//ATTRIBUTE_ACCESSORS(ClassName, PropertyName)中的GAMEPLAYATTRIBUTE_VALUE_INITTER(PropertyName)使其可行
	// 初始值由DataTable实现（以及UGameplayEffect）
	const FAuraGameplayTags& GameplayTags = FAuraGameplayTags::Get();

	/** 优化前：
	FAttributeSignature StrengthDelegate;
	StrengthDelegate.BindStatic(GetStrengthAttribute);//UAuraAttributeSet::GetStrengthAttribute，由于其为static，所有可以省略类作用域
...
/**
 * 这些函数在即将修改属性之前调用。函数旨在实施关于属性值的规则，\n
 * 例如，"生命值必须介于0和最大生命值"之间，并且不得对属性更改触发游戏内响应。
 */
...
/**
 * 在修改属性值后，此函数可立即对更改做出响应。 \n
 * 这通常包括限制属性的最终值或触发对新值的游戏内响应，例如当"生命值"属性降至零时死亡。 \n
 * 主要用于处理GE产生的效果，并非用于修改Attribute
 */
...
void UAuraAttributeSet::HandleIncomingDamage(const FEffectProperties& Props)
{
	...
	if (LocalIncomingDamage > 0.f)
	{
		...
		const bool bFatal = NewHealth <= 0.f; // 伤害是否致命
		if (bFatal)
		{
...
void UAuraAttributeSet::Debuff(const FEffectProperties& Props)
{
	...
	// GE属性：Components
	// 弃用：Effect->InheritableOwnedTagsContainer.AddTag(GameplayTags.DamageTypesToDebuffs[DamageType]);
	...
	// GE属性：Stacking
	Effect->StackingType = EGameplayEffectStackingType::AggregateBySource; // 按源聚合：每个施法者都用于自己的堆栈
	...
	if (const FGameplayEffectSpec* MutableSpec = new FGameplayEffectSpec(Effect, EffectContext, 1.f))
	{
		...
		const TSharedPtr<FGameplayTag> DebuffDamageType = MakeShareable(new FGameplayTag());
...
void UAuraAttributeSet::HandleIncomingXP(const FEffectProperties& Props)
{
	...
	if (Props.SourceCharacter->Implements<UPlayerInterface>() && Props.SourceCharacter->Implements<UCombatInterface>())
	{
		...
		if (NumLevelUps > 0)
		{
			...
			// Attribute Points和Spell Points应该逐级累加
...
{
	// 需要包含"GameplayEffectExtension.h"
	...
	if (IsValid(Props.SourceASC) && Props.SourceASC->AbilityActorInfo.IsValid() && Props.SourceASC->AbilityActorInfo->AvatarActor.IsValid())
	{
		...
		if (IsValid(Props.SourceController)) //不是else if
		{
